...
export default function CleanerPropertiesPage({
  ...
  const cleanerReservations = useMemo(() => {
    return reservations.filter((reservation) => {
      const assignedCleanerId =
        reservation.cleanerId ?? reservation.assignedCleanerId;

      return String(assignedCleanerId) === String(cleanerPortalId);
    });
  }, [reservations, cleanerPortalId]);

  const cleanerHomeIds = useMemo(() => {
    return new Set(
      cleanerReservations
        .map((reservation) => reservation.homeId)
        .filter(Boolean)
        .map(String)
    );
  }, [cleanerReservations]);

  const cleanerHomes = useMemo(() => {
    return homes.filter((home) => {
      const cleanerOwnsProperty =
        String(home.defaultCleanerId ?? "") === String(cleanerPortalId ?? "");

      return cleanerOwnsProperty || cleanerHomeIds.has(String(home.id));
    });
  }, [homes, cleanerHomeIds, cleanerPortalId]);
  ...
  const openMaintenanceCount = workOrders.filter(
    (order) =>
      cleanerHomes.some(
        (home) => String(home.id) === String(order.homeId)
  ...
  async function handleDeleteProperty(home: any) {
    ...
    if (selectedHomeId === String(home.id)) {
  ...
  async function copyCalendarRequest(values: CleanerPropertyFormValues) {
    ...
    try {
      ...
      setCopyStatus("Text message copied. Paste it into your phone or preferred messaging app.");
    } catch {
      ...
      <main className="cleanerPropertySavedPage">
        <section className="cleanerPropertySavedCard">
          ...
          <p className="cleanerPropertySavedLead">
            The property details and cleaning fee have been saved. The
            reservation calendar is still needed from the homeowner.
          </p>
          ...
          <div className="cleanerPropertySavedActions">
            ...
            {savedProperty.ownerPhone && (
              <button
                ...
                type="button"
                onClick={() => openTextRequest(savedProperty)}
              >
          ...
          {copyStatus && (
            <p className="cleanerPropertyRequestCopyStatus">{copyStatus}</p>
          )}
    ...
    <main className="cleanerPropertiesPage">
      ...
      {cleanerHomes.length === 0 ? (
        <section className="cleanerPropertiesFirstProperty">
          ...
          <p>
            Add the home, cleaning workload, homeowner contact information,
            access details, invoices, and maintenance in one place.
          </p>
        ...
        <>
          ...
          >
            ...
            <article className="cleanerPropertiesMetricCard">
              ...
              <div>
                <strong>
                  {connectedCalendars}/{cleanerHomes.length}
                </strong>
                <span>Calendars Connected</span>
              </div>
            </article>
          </section>
          ...
          <section className="cleanerPropertiesToolbar">
            ...
            <span className="cleanerPropertiesResultCount">
              {filteredHomes.length}{" "}
              {filteredHomes.length === 1 ? "property" : "properties"}
            </span>
          </section>

          {filteredHomes.length === 0 ? (
            <section className="cleanerPropertiesEmptyState">
              <div className="cleanerPropertiesEmptyIcon">🔎</div>
              <h2>No matching properties</h2>
              <p>Try searching by property name, owner, city, or address.</p>
            </section>
          ) : (
            <section className="cleanerPropertiesGrid">
              {filteredHomes.map((home) => {
                ...
                const calendarConnected = Boolean(
                  home.calendarFeedUrl || home.iCalUrl
                );
                const cleaningFee =
                  home.cleaningFee ??
                  home.cleanerFee ??
                ...
                const propertyOpenMaintenance = workOrders.filter(
                  (order) =>
                    String(order.homeId) === String(home.id) &&
                    order.status !== "Completed"
                ).length;
                const propertyOutstandingInvoices = Number(
                  home.outstandingInvoiceCount ??
                    home.outstandingInvoices?.length ??
                    0
                );
                  ...
                  <article className="cleanerPropertyCard" key={home.id}>
                    <div className="cleanerPropertyCardImage">
                      {propertyImage ? (
                        ...
                        <div className="cleanerPropertyCardImagePlaceholder">
                          <span>🏡</span>
                    ...
                    <div className="cleanerPropertyCardBody">
                      <div className="cleanerPropertyCardTitleRow">
                        ...
                        <span className="cleanerPropertyActivePill">
                          {home.status || "Active"}
                        </span>
                      </div>
                      ...
                      <div className="cleanerPropertyCardStats">
                        <div>
                          <span>Outstanding Invoices</span>
                          <strong>{propertyOutstandingInvoices}</strong>
                        </div>

                        <div>
                          <span>Open Maintenance</span>
                          <strong>{propertyOpenMaintenance}</strong>
                        </div>
                        ...
                        <div>
                          <span>Calendar</span>
                          <strong>
                            {calendarConnected ? "Connected" : "Needed"}
                          </strong>
                        </div>
                      </div>
